...
public class Sil {
		...
		try {
			...
			if (bulletinNumber != null) {
				...
				if(newBill.getMergedBulletinNumbers() != null) {
					processMergedBills(newBill, session);
				}
				else {
					processBill(newBill,session);
				}
			}
			else {
	...
	public static void processMergedBills(SilBill newBill, Session session) throws Throwable {
		...
		for(SilBill mergedBill : mergedSilBills) {
			Bill processedBill = processBill(mergedBill, session);
	...
	public static Bill processBill(SilBill newSilBill, Session session) throws Throwable {
